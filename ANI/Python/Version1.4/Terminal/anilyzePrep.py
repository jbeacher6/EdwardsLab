...
def modifyLineLengthToSeventy(inputDirectoryParam, outputDirectoryParam):
  ext = ".fna"
  print("Modifying the line lengths of the files located in " + inputDirectoryParam + " to 70 >" + outputDirectoryParam + "seventy/")
  os.system("mkdir " + outputDirectoryParam + "/seventy")#
  for root, dir, files in os.walk(inputDirectoryParam, topdown=True):#
    for f in files:#iterate through all of the files in the tree/output_directory/complete/mod
      if f.endswith(ext):#check if the file ends with .fna
        ...
        os.system("grep -oE '.{1,70}' " + inputDirectoryParam + str(f) + " > " + outputDirectoryParam + "seventy/" + str(f))#reformat the lines to lengths of 70 characters per line and cat to /tree/output/complete/mod2/
...
def main(argv):
  ...
  if i is True:
    ...
    deleteNewLines(inputArg, outputArg)
